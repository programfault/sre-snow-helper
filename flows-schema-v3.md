...
- **自动补全**：编辑器里 `${` 后提示 文件参数 + 全局捕获变量（分组展示），增强项。
...
已解决：
- ~~参数下标 vs 命名~~ → v3.2 采用命名变量，文件 params 优先于全局捕获变量；
- ~~flows 与 groups 的归属~~ → flows 嵌套进组。

仍需注意 / 待确认：

1. **无模板 flow 的承载方式**："Ack only" 用"无 common 的组"承载（组不写 `common:`）——
   是否符合使用习惯？（上一轮遗留，未回复）
2. **未知变量名的处理策略**：建议对齐 Services（未知 `${name}` 渲染为自由文本输入框）+
   校验 warning；若希望更严格（必须声明在 params 里，否则校验报错），实现更简单但灵活性低。
3. **模板间共享单步**：两个模板都要用 ack 时需各写一份；结构预留 `ref:` 扩展，v1 不做。
4. **flow 多自有步骤**：v1 仅 `items` 单步；多步用 `steps:` 列表扩展（结构兼容）。
...
6. **变量名规范**：建议限定 `[A-Za-z0-9_]+`，含空格/特殊字符校验告警（YAML 值里写
   `${my name}` 这类仍能解析，但易踩坑）。
7. **自动补全增强**：`${` 后提示 文件参数 + 全局捕获变量（分组）——建议做，成本低。
