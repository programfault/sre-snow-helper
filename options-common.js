...
const BUNDLE_DOC_PLACEHOLDER =
  [
    ...
    "#   library, type: textarea a multi-line box. Only the params a selected",
    "#   flow actually references (${name} in its steps) render in the panel.",
    ...
    "# An EMPTY value clears the field: `assign_to:` / `assign_to: ~` /",
    "#   `assign_to: \"\"` all send an empty string — validation allows it even",
    "#   for fields whose Form library rows are number/sysid typed.",
    "# `comments` is special: an EMPTY value means \"same as work_notes\"",
    "#   (write the text once) — e.g. `work_notes: done` + `comments: \"\"`.",
    "# Fixed values must match a row in the Form library for that key.",
    "version: 3",
    "",
    "params:",
    ...
    "      state: 2",
    "      work_notes: ack",
    "      # assign_to:      <- empty value = clear the field",
    "",
    "common:",
